...
export function useAudioRecorder({ apiKey, onTranscriptLine, onError }: UseAudioRecorderOptions) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        // Send any remaining audio when stopped mid-interval
        if (chunks.length > 0) {
      ...
      // Every CHUNK_INTERVAL_MS: stop → triggers onstop? No, use timeslice instead
      // timeslice approach: ondataavailable fires every N ms
      recorder.start(CHUNK_INTERVAL_MS)

      // But with timeslice, chunks accumulate between fires — send each timeslice chunk
      recorder.ondataavailable = (e) => {
        if (e.data.size > 0) {
